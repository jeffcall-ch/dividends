...
class DividendProcessor(object):
    ...
    def get_dividend_dates_values_of_year(self, year):
        ...
        return dividends_filtered.index.tolist()
    ...
    def is_dividend_exist_plus_minus_some_days_a_year_ago(self):
        dividend_dates_current_year = self.get_dividend_dates_values_of_year(self.this_year)
        mask = (self.dividends_raw.index <= (str(self.this_year-1)+'-12-31'))
        dividends_filtered=self.dividends_raw.loc[mask]
        dividend_dates_all = dividends_filtered.index.tolist()
        matched_dates = []
        for date in dividend_dates_current_year:
            # a year ago plus 15 days
            datehigh = date - timedelta(days=350)
            # a year ago minus 15 days
            datelow = date - timedelta(days=380)
            for olddate in dividend_dates_all:
                if datelow < olddate < datehigh:
                    matched_dates.append(olddate)
        print (f"matched dividend dates a year ago of current year : {matched_dates}")
        if len(dividend_dates_current_year) == len(matched_dates):
            return True
        else:
            return False
...
dataproc = DividendProcessor('wlkp')
...
print (f"div dates of 2018 {dataproc.get_dividend_dates_values_of_year(2018)}")
print (f"div dates of 2019 {dataproc.get_dividend_dates_values_of_year(2019)}")
print (f"div dates of 2020 {dataproc.get_dividend_dates_values_of_year(2020)}")
print (f"div dates of 2021 {dataproc.get_dividend_dates_values_of_year(2021)}")

print (f"div exists a year ago plus minus 15 days {dataproc.is_dividend_exist_plus_minus_some_days_a_year_ago()}")
