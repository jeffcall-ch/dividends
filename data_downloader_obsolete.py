...
def save_one_dividend_to_csv(symbol):
    ...
    dividends_raw.to_csv(output_path, mode='a', header=not os.path.exists(output_path))

def save_one_cf_statement_to_csv(symbol):
    # freeCashFlow
    # dividendsPaid
    ...
    # export to csv. Use header only for the first time, then only append new rows
    output_path = r'C:\Users\50000700\Python\Python_repos\dividends\excel_files\cf_statements.csv'
    cfStatement.to_csv(output_path, mode='a', header=not os.path.exists(output_path))

def save_one_income_statement_to_csv(symbol):
    # eps
    # epsdiluted
    ...
    # export to csv. Use header only for the first time, then only append new rows
    output_path = r'C:\Users\50000700\Python\Python_repos\dividends\excel_files\income_statements.csv'
    incomeStatement.to_csv(output_path, mode='a', header=not os.path.exists(output_path))
...
# ticker_list = ["ADP","ADM", "AFL", "FASZ", "GECI"]
...
for ticker in ticker_list:
    ...
    if request_counter >= 10 :
        ...
        if (time.time() - start_time) < 1 :
            ...
            start_time = time.time()
    try:
        # save_one_dividend_to_csv(ticker)
        save_one_cf_statement_to_csv(ticker)
        save_one_income_statement_to_csv(ticker)
    except:
        print ("ERROR")
...
with open(r'C:\Users\50000700\Python\Python_repos\dividends\excel_files\failed_cf_income.csv', 'w', newline='') as failed_file:
